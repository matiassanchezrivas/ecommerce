...
class MenuProduct extends React.Component {
  constructor() {
    ...
    this.addToCart=this.addProductCart.bind(this)
...
<<<<<<< HEAD
      ...
      cat.push(<p><button key={product.categoria} onClick={(e) => this.categoryFilterList(product.categoria)}>{product.categoria}</button></p>)
        ...
        if(product.title.toLocaleLowerCase().indexOf(filter.toLowerCase()) > -1){rows.push(<Grid item xs={4} key={product.title}>
        ...
        <div className={classes.imagen}>
                    ...
                    <div className={classes.title}>
                        <h4>{product.title}</h4>
                        <Button className={classes.button} onClick={()=>this.addProductCart(product)}>
                        comprar
=======
      if (this.state.filter !== null) {
        console.log("aplicando filtro")
        db.forEach((product) => {
          cat.push(<p><button key={product.categoria} onClick={(e) => this.categoryFilterList(product.categoria)}>{product.categoria}</button></p>)
          const filter = this.state.filter
          if (product.Nombre.toLocaleLowerCase().indexOf(filter.toLowerCase()) > -1) {
